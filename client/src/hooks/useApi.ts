...
export const api = {
  ...
  // Display mode
  getDisplayMode: (userId: number) =>
    apiFetch<{ displayMode: boolean }>(`/users/${userId}/display-mode`),
  setDisplayMode: (userId: number, enabled: boolean) =>
    apiFetch<{ displayMode: boolean }>(`/users/${userId}/display-mode`, { method: 'PUT', body: JSON.stringify({ displayMode: enabled }) }),
  dashboardDisplay: () =>
    apiFetch<any>('/dashboard/display'),
  ...
  updatePasswordless: (userId: number, data: { passwordless: number }) =>
    apiFetch<{ success: boolean }>(`/users/${userId}/passwordless`, { method: 'PUT', body: JSON.stringify(data) }),
  updateDisplayMode: (userId: number, data: { displayMode: number }) =>
    apiFetch<{ success: boolean }>(`/users/${userId}/display-mode`, { method: 'PUT', body: JSON.stringify(data) }),
};

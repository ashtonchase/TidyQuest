...
// Display mode endpoint - returns tasks grouped by user for display page
router.get('/display', (req: AuthRequest, res: Response) => {
   const user = db.prepare(
     'SELECT id, username, displayName, role, avatarColor, avatarType, avatarPreset, avatarPhotoUrl, coins, currentStreak, goalCoins, goalStartAt, goalEndAt, lastActiveDate, language FROM users WHERE id = ?'
   ).get(req.userId) as any;
   const vacation = getGlobalVacation();

   // Get all users with displayMode enabled
   const displayUsers = db.prepare(
     'SELECT id, displayName, avatarColor, avatarType, avatarPreset, avatarPhotoUrl FROM users WHERE displayMode = 1 ORDER BY displayName'
   ).all() as any[];
   
   const displayUserIds = displayUsers.map((u: any) => u.id);
   ...
   // Group tasks by user who has display mode enabled
   const userTasks = new Map<number, { incomplete: any[]; completed: any[] }>();
   for (const userId of displayUserIds) {
     userTasks.set(userId, { incomplete: [], completed: [] });
   }
   ...
   for (const room of rooms) {
     ...
     for (const t of tasks) {
       ...
       // Check if any assignee has display mode enabled
       const assigneesWithDisplayMode = effectiveIds.filter((id: number) => displayUserIds.includes(id));

       if (assigneesWithDisplayMode.length === 0) continue;
       ...
       const taskData = {
         ...
         health,
         dueDate: new Date(dueDateTs).toISOString(),
         dueInDays: t.onDemand ? 0 : Math.max(0, Math.floor((dueDateTs - nowTs) / 86400000)),
         roomName: room.name,
         roomColor: room.color,
         roomAccent: room.accentColor,
         assignedUsers: taskAssigneesForTask.map((a: any) => ({
       ...
       // Check if completed today by any assignee
       const completedToday = completedTodayByTask.get(t.id);
       const isCompletedByAssignee = completedToday && effectiveIds.includes(completedToday.userId);

       if (isCompletedByAssignee) {
         // Add to completed list for each assignee with display mode
         for (const userId of assigneesWithDisplayMode) {
           userTasks.get(userId)!.completed.push({
             ...taskData,
             completedBy: completedToday,
           });
         }
       } else {
         // Add to incomplete list for each assignee with display mode
         for (const userId of assigneesWithDisplayMode) {
           userTasks.get(userId)!.incomplete.push(taskData);
         }
       }
     }
   }

   // Sort incomplete tasks by health (lowest first), then due date
   // Sort completed tasks by completion time (most recent first)
   for (const [userId, data] of userTasks) {
     data.incomplete.sort((a: any, b: any) => {
       if (a.health !== b.health) return a.health - b.health;
       return new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime();
     });
     data.completed.sort((a: any, b: any) => {
       return new Date(b.completedBy.completedAt).getTime() - new Date(a.completedBy.completedAt).getTime();
     });
   }
